...
执行记录面板用于在 VLA 推理过程中自动/手动记录每条语言指令的执行情况，包括任务名、指令内容、起止时间、耗时、评分和备注。记录持久化在浏览器 `localStorage` 中，可导出 CSV 或保存到log文件夹。

## 面板入口

点击工具栏 **Log** 按钮可展开/收起执行记录面板。面板作为主布局的第四列显示，列宽可通过列间分隔条拖拽调整。
...
| 列 | 内容 |
|---|---|
| **Task#** | `#序号` + 任务名（截断）+ 子任务编号，hover 显示完整信息 |
| **Instruction** | 指令文本（截断），hover 显示全文 |
| **Time** | 开始时间（HH:MM:SS） |
...
每条记录有全局自增的数字 ID（`#1, #2, #3...`），在 Task# 列灰色小字显示。Clear All 后 ID 从 `#1` 重新开始。
