...
def get_record_data_config():
    ...
    config.save_path = "./output/test" ## record data save path
    config.info = ConfigDict(allow_dotted_keys=True)
    config.info.codebase_version = "v2.0" ## lerobot dataset version
    config.info.robot_type = "a2d" ## robot type

    # info.json init config
