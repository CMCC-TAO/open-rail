...
class VLAClientAsync():
    ...
    def _inference_first(self):
        ...
        if data is not None:
            ...
            result = self._request_inference(data, timeout_ms=self.config.vla_zmq.timeout * 4)
    ...
    def _inference_step(self):
        ...
        if data is not None:
            ...
            # Send data for inference and wait for results
            result = self._request_inference(data, timeout_ms = self.config.vla_zmq.timeout)
            if result is None:
